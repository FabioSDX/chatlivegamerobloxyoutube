        ...
        if (pick) {
          ...
          if (isMegaClearingRow(pickRow)) {
            ...
            if (mz >= 0 && !megaClearedZones[mz]) {
              ...
              if (isZombieGate) {
                // ── ZOMBIE GATE: spawn zombies + zombie bosses ──
                var zMegaNum = Math.min(MAX_ZOMBIES_MEGA, 20 + Math.floor(mz * 5));
                for (var mi = 0; mi < zMegaNum; mi++) {
                ...
                // Spawn Zombie Boss Gate at the bottom
                var zbRow = mStart + MEGA_CLEARING_HEIGHT - 10;
                var zbY = zbRow * TILE + TILE / 2;
            ...
            // Continuous reinforcement every 90 frames (~1.5s)
            if (mz >= 0 && megaClearedZones[mz]) {
              megaClearedZones[mz + '_tick'] = (megaClearedZones[mz + '_tick'] || 0) + 1;
              var isZGate = megaClearedZones[mz + '_type'] === 'zombie';
              if (megaClearedZones[mz + '_tick'] % 90 === 0) {
                ...
                if (isMegaClearingRow(ryRow)) {
                  if (isZGate && zombies.length < MAX_ZOMBIES_MEGA) {
                    spawnZombie(rx, ry);
                  } else if (!isZGate && bats.length < MAX_BATS_MEGA) {
    ...
    function checkZombieBossBarrierCollisions() {
      ...
      var segments = [];
      var wallL = TILE, wallR = (COLS - 1) * TILE;
      segments.push({ x1: wallL, y1: bosses[0].y, x2: bosses[0].x, y2: bosses[0].y });
      for (var qi = 0; qi < bosses.length - 1; qi++) {
        segments.push({ x1: bosses[qi].x, y1: bosses[qi].y, x2: bosses[qi + 1].x, y2: bosses[qi + 1].y });
      }
      segments.push({ x1: bosses[bosses.length - 1].x, y1: bosses[bosses.length - 1].y, x2: wallR, y2: bosses[bosses.length - 1].y });
      var BARRIER_THICK = 30;
      var allPicks = [pick].concat(userPicks, extraPicks);
      for (var pi = 0; pi < allPicks.length; pi++) {
        var p = allPicks[pi];
        ...
        for (var si = 0; si < segments.length; si++) {
          ...
          if (Math.abs(dy) < BARRIER_THICK && p.vy > 0.5) {
            ...
            for (var sp = 0; sp < 3; sp++) {
              parts.push({ x: p.x, y: lineY, vx: (Math.random() - 0.5) * 4, vy: -1 - Math.random() * 2, life: 1, dec: 0.08, c: '#44ff00', r: 2 + Math.random() * 2 });
            }
          }
        }
      }
    ...
    function updateZombies() {
      ...
      for (var i = zombies.length - 1; i >= 0; i--) {
        ...
        // ── Zombie Boss: stay near home, just patrol slowly ──
        if (z.isZBoss) {
          ...
          z.x += z.vx;
          z.dir = z.vx > 0 ? 1 : -1;
          // Apply gravity, stay grounded
          z.vy += ZG; z.y += z.vy;
          var bFootRow = Math.floor((z.y + ZOMBIE_SIZE * ZBOSS_SCALE * 0.5) / TILE);
          var bGround = getCell(bFootRow, Math.floor(z.x / TILE));
          if (bGround && bGround.t !== E && z.vy >= 0) {
