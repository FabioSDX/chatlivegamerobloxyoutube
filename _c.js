    ...
    function checkConnection() {
      ...
      if (!_liveChatId) {
        ...
        if (txtApi) { txtApi.textContent = 'API: OK'; txtApi.style.color = '#00ff88'; }
        if (dotLive) { dotLive.classList.remove('online'); dotLive.classList.add('warning'); }
        ...
        .then(function(data) {
          ...
          if (data.error) {
            ...
            if (errCode === 403 || errMsg.toLowerCase().indexOf('quota') !== -1) {
              console.warn('[checkConnection] Quota exceeded or 403:', errMsg);
              if (dotApi) dotApi.classList.remove('online');
              if (txtApi) { txtApi.textContent = 'API: Quota exceeded'; txtApi.style.color = '#ff4444'; }
              if (dotLive) dotLive.classList.remove('online');
              if (txtLive) { txtLive.textContent = 'Live: paused (quota)'; txtLive.style.color = '#888'; }
              // Stop polling to avoid wasting more quota
              if (_apiCheckInterval) { clearInterval(_apiCheckInterval); _apiCheckInterval = null; }
              rotateApiKey(_activeApiKey, errMsg);
              return;
            }
            // Live ended or invalid chat
            ...
            // General error
            if (dotApi) dotApi.classList.remove('online');
            if (txtApi) { txtApi.textContent = 'API: Error'; txtApi.style.color = '#ff4444'; }
            rotateApiKey(_activeApiKey, errMsg || 'api_error');
            return;
          }
